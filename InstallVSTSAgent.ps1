...
Write-Output "Current folder: $currentLocation" -verbose
...
$agentTempFolderName = Join-Path $env:temp ([System.IO.Path]::GetRandomFileName()) #Join-Path $agentInstallationPath "tmp" #
...
#To avoid SSL/TLS secure channel error
...
do {
    try {
		...
		#Create file and write exceptions to it to debug the issue - uncomment the below line and make changes according to your setup
		#New-Item -Path $agentInstallationPath -Name "testfile1.txt" -ItemType "file" -Value $exceptionText
        $retries++
...
# Create a folder for the build work  $WorkFolder
